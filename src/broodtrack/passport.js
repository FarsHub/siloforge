...
function buildPassportHTML(sale){
  ...
  const feedLine=batch
    ? getBroodFeedProgramme(getBirdType(batch))
        .filter(p=>p.type)
        .map(p=>`<span class="pp-chip">${rdEsc(p.type)} <em>${p.maxDay>=999?'to sale':'to d'+p.maxDay}</em></span>`)
        .join('')
    : '';
  ...
  <div class="rd rd-pass">
    ...
    ${feedLine?`
      <div class="rd-rule"></div>
      <div class="vx-sec"><div class="rd-lbl">Reared on</div></div>
