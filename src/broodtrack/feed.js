...
function renderFeed(){
  ...
  el.innerHTML=`<div class="topbar"><div><h1>Feed</h1><small>Daily usage · store &amp; reorder</small></div></div>
...
function feedUsageBody(){
  ...
  if(!_activeBatchId||!DB.getBatches().find(b=>b.id===_activeBatchId))return getBatchSelectPrompt();
  const batch=DB.getBatches().find(b=>b.id===_activeBatchId);
  const recs=DB.getFeed().filter(r=>r.batch_id===_activeBatchId).sort((a,b)=>b.date.localeCompare(a.date));
  const todayRec=recs.find(r=>r.date===today);
  const totalFeed7=recs.filter(r=>{const d=new Date(today);d.setDate(d.getDate()-7);return r.date>d.toISOString().slice(0,10);}).reduce((s,r)=>s+(r.feed_kg_used||0),0);
  ...
  const feedRateG=getFeedRateG(batch,ageDays);
  const logHtml=`<div class="card">
    <div class="card-title">${todayRec?'Today\'s Feed Log ✓':'Log Feed Usage'}</div>
    ${todayRec?`
      ${(()=>{const vr=todayRec.feed_req_kg>0?((todayRec.feed_kg_used-todayRec.feed_req_kg)/todayRec.feed_req_kg*100):0;
        const vc=Math.abs(vr)>15?'var(--red)':Math.abs(vr)>10?'var(--amber)':'var(--g2)';
        return`<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:10px">
          <div class="kpi-sm"><div class="kpi-val-sm" style="color:var(--p2)">${todayRec.feed_kg_used}</div><div class="kpi-lbl-sm">kg Used</div></div>
        ...
        <div style="font-size:12px;color:var(--gray);margin-bottom:10px">${canonFeedType(todayRec.feed_type)} · ${todayRec.feed_phase}</div>`;})()}
      <div style="display:flex;gap:8px">
        <button class="btn btn-amber btn-sm" style="flex:1" onclick="openFeedForm('${todayRec.id}')">Edit</button>
        <button class="btn btn-danger btn-sm" style="flex:1" onclick="delFeedLog('${todayRec.id}')">Delete</button>
      </div>`:`<button class="btn btn-primary" onclick="openFeedForm()">+ Log Feed Today</button>`}
  </div>`;
  const histHtml=recs.filter(r=>r.date!==today).slice(0,30).map(r=>{
    ...
    return`<div class="list-item">
      <div><div style="font-weight:700;font-size:14px">${fmtDate(r.date)} <span style="font-size:11px;color:var(--gray)">${canonFeedType(r.feed_type)}</span></div>
        <div style="font-size:12px;color:var(--gray);margin-top:2px">${r.feed_kg_used} kg used · Req: ${r.feed_req_kg} kg · ${r.feed_phase}</div>
      ...
      <div style="display:flex;gap:5px">
        ...
        <button class="btn btn-danger btn-sm" onclick="delFeedLog('${r.id}')">✕</button>`}
      </div></div>`;
  }).join('');
  return getBatchCtxBar()+`
    <div class="kpi-row-3">
      <div class="kpi"><div class="kpi-val">${totalFeed7.toFixed(1)}</div><div class="kpi-lbl">kg Used (7d)</div></div>
      <div class="kpi"><div class="kpi-val">${feedRateG}</div><div class="kpi-lbl">g/Bird/Day<br><span style="font-size:9px;color:var(--p3);font-weight:500">${phase.label}</span></div></div>
      <div class="kpi"><div class="kpi-val" style="color:var(--p2)">${getBatchBirdCount(batch)}</div><div class="kpi-lbl">Live Birds</div></div>
    </div>
    ${logHtml}
    <div class="sec-hdr">History</div>
    <div class="card" style="padding:0;overflow:hidden">${histHtml||'<div class="empty" style="padding:24px"><p>No feed records yet for this batch.</p></div>'}</div>
    <div style="height:12px"></div>`;
}

// ── Feed Store tab ──────────────────────────────────────────────────────
// Not gated on a selected pen: there is one store, and whoever is holding the
// phone in it should be able to enter a count without first picking a batch.
...
function feedStoreBody(){
  ...
  if(rows.length===0){
    return `<div class="empty" style="padding:28px 24px">
        ...
        <p>Count what is in the store and the app works out whether it covers the stage each batch is on, and how many bags to buy — no more counting and dividing by hand.</p>
      </div>${actions}`;
  }
  ...
  const cards=rows.map(r=>{
    ...
    const bg=r.state==='urgent'?'var(--redBg)':r.state==='soon'?'var(--amberBg)'
      :r.state==='idle'?'#f5f5f5':'var(--p5)';
    const fg=r.state==='urgent'?'#7f1d1d':r.state==='soon'?'#7d4e00'
      :r.state==='idle'?'var(--gray)':'var(--p1)';
    ...
    if(r.state==='idle'){
      ...
      sub=r.todayKg>0
        ?`Eating <b>${r.todayKg.toFixed(1)} kg/day</b> today across ${r.penCount} batch${r.penCount===1?'':'es'} — rises as the flock ages`
    ...
    const footer=r.state==='idle'
      ...
      : r.buyBags>0
        ...
        : `<div style="font-size:12px;color:var(--g2);font-weight:700;border-top:1px solid #f0f0f0;padding-top:10px">✓ ${r.openEnded?`Enough for the next ${targetDays} days`:'Nothing to buy for this stage'}</div>`;
    ...
    return `<div class="card" style="${open?'':'padding:14px 16px'}">
      ...
      <div style="background:${bg};border-radius:10px;padding:11px 13px;margin-bottom:10px">
        ...
        ${sub?`<div style="font-size:12px;color:var(--p1);margin-top:3px">${sub}</div>`:''}
      </div>
      ${footer}
  ...
  const shrinkHtml=allCounts.length===0?'':`
    <div style="margin:0 16px 8px;background:${lossKg<-1?'var(--amberBg)':'var(--p5)'};border-radius:10px;padding:10px 13px;font-size:12px;color:${lossKg<-1?'#7d4e00':'var(--p1)'}">
      ${lossKg<-1
...
// ── Feed purchase ───────────────────────────────────────────────────────
// Entered once, in bags, and it writes the matching Feed expense so the store
// and the books cannot drift apart through double entry.
function openFeedPurchase(editId,presetType){
  ...
  const type=canonFeedType(rec?.feed_type||presetType||getFeedStockByType()[0]?.feed_type)||BROOD_FEED_TYPES[1];
  openModal(`<div class="modal-ttl">${rec?'Edit':'Record'} Feed Purchase <button class="modal-x" onclick="closeModal()">×</button></div>
    ...
    <div class="field"><label>Feed Type</label>
      <select id="fp_type" onchange="calcFeedPurchase()">${BROOD_FEED_TYPES.map(t=>`<option value="${t}" ${t===type?'selected':''}>${t}</option>`).join('')}</select></div>
    ${active.length?`<div class="field"><label>Batch <span style="color:var(--gray);font-weight:400">— optional, for the expense</span></label>
      <select id="fp_batch"><option value="">Not batch-specific</option>${active.map(b=>`<option value="${b.id}" ${rec?.batch_id===b.id?'selected':''}>${b.name}</option>`).join('')}</select></div>`:'<input type="hidden" id="fp_batch" value="">'}
    ...
    <div id="fp_prev" style="background:var(--p5);border-radius:8px;padding:9px 12px;font-size:12px;color:var(--p1);margin-bottom:12px"></div>
...
function saveFeedPurchase(editId){
  ...
  const batch_id=document.getElementById('fp_batch')?.value||'';
  ...
  const id=editId||uid();
  const rec={id,date,kind:'purchase',feed_type,bags,loose_kg,kg,bag_kg,cost_ngn,batch_id,
    batch_name:batch_id?(DB.getBatches().find(b=>b.id===batch_id)?.name||''):'',
...
// Keeps one Feed expense in step with the purchase record. Mutates
...
function syncFeedPurchaseExpense(rec){
  ...
  const payload={date:rec.date,category:'Feed',amount_ngn:rec.cost_ngn,amount_usd:0,
    batch_id:rec.batch_id||'',batch_name:rec.batch_name||'',notes,feed_stock_id:rec.id};
...
function openFeedCount(editId,presetType){
  ...
  const type=canonFeedType(rec?.feed_type||presetType||stocked[0]?.feed_type)||BROOD_FEED_TYPES[1];
  openModal(`<div class="modal-ttl">${rec?'Edit':'Enter'} Stock Count <button class="modal-x" onclick="closeModal()">×</button></div>
    ...
    <div class="field"><label>Feed Type</label>
      <select id="fc_type" onchange="calcFeedCount()">${BROOD_FEED_TYPES.map(t=>`<option value="${t}" ${t===type?'selected':''}>${t}</option>`).join('')}</select></div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
    ...
    <div id="fc_prev" style="background:var(--p5);border-radius:8px;padding:10px 12px;font-size:12px;color:var(--p1);margin-bottom:12px"></div>
...
function calcFeedCount(){
  ...
  if(row&&row.demand){
    ...
    coverTxt=`<div style="margin-top:6px;font-weight:800;font-size:13px;color:${urgent?'var(--red)':'var(--p1)'}">${line}</div>
...
function showFeedCountResult(feedType){
  ...
  openModal(`<div class="modal-ttl">${feedType} <button class="modal-x" onclick="closeModal()">×</button></div>
    <div style="background:${bg};border-radius:10px;padding:14px;margin-bottom:12px;text-align:center">
      ...
      <div style="font-size:12px;font-weight:700;color:var(--p1);text-transform:uppercase;letter-spacing:.4px;margin-top:4px">${big.lbl}</div>
      <div style="font-size:12px;color:var(--p1);margin-top:6px">${r.headline}</div>
    </div>
    <div style="font-size:13px;color:var(--p1);line-height:1.7;margin-bottom:14px">
...
function delFeedStock(id){
  ...
  openModal(`<div class="modal-ttl">Delete? <button class="modal-x" onclick="closeModal()">×</button></div>
    <p style="font-size:14px;color:var(--gray);margin-bottom:14px">
      ...
      ${linked?`<br><br>The linked <b>${fmtMoney(linked.amount_ngn)}</b> Feed expense is removed too.`:''}
...
function openFeedForm(editId){
  ...
  const batch=DB.getBatches().find(b=>b.id===_activeBatchId); if(!batch)return;
  const ageDays=batchAgeInDays(batch)||0;
  const birdType=getBirdType(batch);
  const phase=getFeedPhaseForBatch(batch,ageDays);
  const farm=DB.getFarm();
  ...
  const defReq=(birds*feedRateG/1000).toFixed(1);
  const typeOpts=BROOD_FEED_TYPES.map(t=>`<option value="${t}" ${canonFeedType(rec?.feed_type)===t?'selected':''}>${t}</option>`).join('');
  openModal(`<div class="modal-ttl">${rec?'Edit':'Add'} Feed Entry <button class="modal-x" onclick="closeModal()">×</button></div>
    <div style="background:#e8f4fd;border-left:3px solid #4895ef;padding:8px 12px;border-radius:0 6px 6px 0;font-size:12px;color:#1a5fa8;margin-bottom:12px">
      📅 Logging yesterday's feed? Change the date below.<br><span style="margin-top:4px;display:block">⚖️ Phase: <b>${phase.label}</b> · Rate: <b>${feedRateG} g/bird</b> · Req for ${birds} birds: <b>${defReq} kg</b></span>
    </div>
    <div class="field"><label>Date</label><input type="date" id="ff_date" value="${rec?rec.date:today}" min="${lockMinDate()}" max="${today}"></div>
    <div class="field"><label>Feed Type</label><select id="ff_type">${typeOpts}</select></div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
      <div class="field"><label>kg Used</label><input type="number" id="ff_used" value="${rec?.feed_kg_used||''}" min="0" step="0.5" placeholder="${defReq}"></div>
    ...
    <div class="field"><label>Notes <span style="color:var(--gray);font-weight:400">— optional</span></label><textarea id="ff_notes" placeholder="e.g. Switched bags, birds eating well">${rec?.notes||''}</textarea></div>
...
function saveFeedLog(editId){
  ...
  if(!lockGuard(document.getElementById('ff_date').value,existing?.date))return;
  const feed_rate_g=existing?.feed_rate_g||getFeedRateG(batch,ageDays);
  const rec={id:editId||uid(),date:document.getElementById('ff_date').value,
    batch_id:_activeBatchId,batch_name:batch.name,breed:batch.breed,bird_type:birdType,
    feed_type:document.getElementById('ff_type').value,
    feed_phase:phase.label,
    feed_kg_used:parseFloat(document.getElementById('ff_used').value)||0,
    feed_req_kg:parseFloat(document.getElementById('ff_req').value)||0,
    feed_rate_g,
    notes:document.getElementById('ff_notes').value.trim()};
  if(editId){DB.updFeed(editId,rec);}
  else{DB.getFeed().filter(r=>r.batch_id===_activeBatchId&&r.date===rec.date).forEach(r=>DB.delFeed(r.id));DB.addFeed(rec);}
